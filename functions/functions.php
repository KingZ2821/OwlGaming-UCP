...
function isPlayer($rank, $groups) {
    return array_search($rank, explode(',', $groups)) !== false;
}

function getUserTitle($groups) {
    if (isPlayer(HEADADMIN, $groups)) {
        return 'Head Admin ';
    } else if (isPlayer(LEADADMIN, $groups)) {
        return 'Lead Admin ';
        ...
        return 'Scripter ';
    } else if (isPlayer(SUPPORTER_MANAGER, $groups)) {
...
function isPlayerHeadAdmin($text) {
    return isPlayer(HEADADMIN, $text);
}

function isPlayerLeadAdmin($text) {
    return isPlayerHeadAdmin($text) or isPlayer(LEADADMIN, $text);
}
...
function isPlayerSupporter($text) {
    return isPlayerTrialAdmin($text) or isPlayer(SUPPORTER, $text) or isPlayer(SUPPORTER_MANAGER, $text);
}

function isPlayerSupporterManager($text) {
    return isPlayerTrialAdmin($text) or isPlayer(SUPPORTER_MANAGER ,$text);
}

function isPlayerVCTLeader($text) {
    return isPlayer(VCT_LEADER, $text);
}
...
function getBanningInfo($connection, $userid, $appstate, $activated, $groups) {
    ...
    $ban = $conn->query_first("SELECT bans.id AS id, bans.admin AS admin, accounts.username AS username, bans.reason AS reason FROM bans LEFT JOIN accounts ON bans.admin=accounts.id WHERE account='" . $userid . "' AND (bans.until IS NULL OR bans.until > NOW()) LIMIT 1");
...
function getAccountInfo($userID, $groups) {
	...
	FROM `accounts` `a` WHERE `a`.`id`='" . $dbMTA->escape($userID) . "' LIMIT 1";
        $userRow = $dbMTA->query_first($mQuery1);
        $hoursplayed = $dbMTA->query_first("SELECT SUM(hoursplayed) AS hoursplayed FROM characters WHERE account='" . $dbMTA->escape($userID) . "' ")['hoursplayed'];
...
function getCharactersFromAccount($userID, $hideThisChar) {
    ...
    $dbMTA = new Database("MTA");
    $dbMTA->connect();
    $preparedText = "SELECT `active`, `age`, `lastarea`, `id`, `charactername`,`cked`,`skin`, `gender` FROM `characters` WHERE `account`='" . $dbMTA->escape($userID) . "'";
...
function get_host(){
    ...
    return $currentUrl = $protocol . '://' . $host;
}
...
function getReportTypes( $type = false ) {
    $reports = array( 
        '1' => array( 'name' => 'Issue with another player', 'shortname' => 'PLY', 'color' => '#D60606' ),
        '2' => array( 'name' => 'Interior Issue', 'shortname' => 'INT', 'color' => '#FF7E00' ),
        '3' => array( 'name' => 'Item Issue', 'shortname' => 'ITM', 'color' => '#FF7E00' ),
        '4' => array( 'name' => 'General Question', 'shortname' => 'SUP', 'color' => '#46C81E' ),
        '5' => array( 'name' => 'Vehicle Related Issues', 'shortname' => 'VEH', 'color' => '#FF7E00' ),
        '6' => array( 'name' => 'Vehicle Build/Import Requests', 'shortname' => 'VCT', 'color' => '#B007ED' ),
        '7' => array( 'name' => 'Scripting Question', 'shortname' => 'SCRT', 'color' => '#947E0C' ),
    );
    return is_numeric($type) ? $reports[$type] : $reports;
}
