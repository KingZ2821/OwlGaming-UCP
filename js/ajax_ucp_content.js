...
function ajax_load_acc_info() {
    $.post("../ajax/ajax_get_acc_info.php", {
    }, function (stuffabc) {
        $('#acc_info').html(stuffabc);
    });
}

function ajax_load_char_info() {
    $.post("../ajax/ajax_get_char_info.php", {
    }, function (stuffxyz) {
        $('#char_info').html(stuffxyz);
    });
}
...
function stat_transfer_step2_validate(loadto) {
    var selectedTo = $('#toCharPreviewSelected').val();
    var selectedFrom = $('#fromCharPreviewSelected').val();
    if (selectedTo && selectedTo > 0 && selectedFrom && selectedFrom > 0) {
        $('#validateText').html("Now, let's choose somethings to transfer!");
        ajax_load_source_character_assets(selectedFrom, selectedTo);
    }
}
...
function ajax_load_source_character_assets(charid, target_id) {
    $('#source_char_assets').html('<center><img src="../images/loading3.gif"/><p><b>&nbsp;&nbsp;Loading..</b></p></center>');
    $.post("../ajax/ajax_stat_transfer_load_source_character_assets.php", {
        charid: charid,
        target_id: target_id,
    }, function (data) {
        $('#source_char_assets').html(data);
...
function mouseClickTransferIcon() {
    ...
    $(document).ready(function () {
        ...
        var slot_veh = $('input[type="checkbox"][name="maxvehicles"]:checked').val();
        var slot_int = $('input[type="checkbox"][name="maxinteriors"]:checked').val();
        var slot_skin = $('input[type="checkbox"][name="max_clothes"]:checked').val();
...
function ajax_change_password() {
    if ($('#changepass').val() == "Change password") {
        ...
        if ($('#curpass').val().length < 1) {
            errorText += '*Current password must not be empty.\n';
        }
...
function switchToFactionPanel(charid, factid) {
    //$("#char_info_mid").append('<div id="overlay_loading"><center><img src="../images/loading3.gif"/><p><b>&nbsp;&nbsp;Loading..</b></p></center></div>');
    //return alert(charid);
    $.post("../ajax/ajax_get_char_details.php", {
...
function enable_two_factor(userid, code) {
    if (code == 1) {
        if ($('#enableTwoFactor').val() == "Enable Two-Factor Authentication") {
            $('#enableTwoFactor').val("Enabling..");
            $.post("../ajax/ajax_load_acc_settings.php", {
                step: 'load_twofactor_setup',
                userid: userid,
            }, function (data) {
                $('#twofactor_details').slideUp(0);
                $('#twofactor_details').html(data);
                $('#twofactor_details').slideDown(500);
            });
        }
    } else if (code == 2) {
        if ($('#twofactor_finalize').val() == "Save") {
            $('#twofactor_finalize').val("Saving..");
            var key = $('#twofactor_confirm_code').val();
            if (isNaN(key) || key.length < 6) {
                alert("Code generated by your Google Authenticator must always be in 6 ditigals length.");
                $('#twofactor_finalize').val("Save");
            } else {
                $.post("../ajax/ajax_load_acc_settings.php", {
                    step: 'finalize_twofactor_setup',
                    userid: userid,
                    key: key,
                }, function (data) {
                    if (data == "Code Validation was incorrect!") {
                        alert(data);
                        $('#twofactor_finalize').val("Save");
                    } else if (data == "Two-Factor Authentication has been successfully enabled!") {
                        alert(data);
                        ajax_load_acc_settings();
                    } else {
                        $('#twofactor_details').slideUp(0);
        ...
        if ($('#enableTwoFactor').val() == "Disable Two-Factor Authentication") {
            $('#enableTwoFactor').val("Disabling..");
            $.post("../ajax/ajax_load_acc_settings.php", {
                step: 'load_twofactor_removal',
                userid: userid,
            }, function (data) {
                $('#twofactor_details').slideUp(0);
                $('#twofactor_details').html(data);
        ...
        if ($('#twofactor_remove').val() == "Remove") {
            $('#twofactor_remove').val("Removing..");
            var key = $('#twofactor_confirm_code').val();
            if (false && isNaN(key) || key.length < 6) {
                alert("Code generated by your Google Authenticator must always be in 6 ditigals length.");
                $('#twofactor_remove').val("Remove");
            } else {
                $.post("../ajax/ajax_load_acc_settings.php", {
                    step: 'remove_twofactor',
                    userid: userid,
                    key: key,
                }, function (data) {
                    if (data == "Code Validation was incorrect!") {
                        alert(data);
                        $('#twofactor_remove').val("Remove");
                    } else if (data == "Two-Factor Authentication has been successfully disabled!") {
                        alert(data);
                        ajax_load_acc_settings();
                    } else {
                        $('#twofactor_details').slideUp(0);
