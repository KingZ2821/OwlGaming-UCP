...
function load_my_tickets(limit, start, load_to) {
...
function client_submit_ticket(type1) {
    if ($('#btn_submit_ticket').val() == "Create") {
        ...
        if (type1 == 2) {
            ...
            subj = $('#reportedstaff').val() + " - " + subj;
            cont = "<b>List the players those were involved or witnessed:</b><br>"
                    + $('#involvedcharacters').val() +"<br>"
                    +"<br><b>Date of incident:</b><br>"
                    + $('#date').val() +"<br>"
                    +"<br><b>Elaborate on the situation along with possible evidence:</b><br>"
                    + $('#story').val() +"<br>"
...
function remove_subs(tcid, sub, subname){
    if (confirm("Are you sure?")) {
        $.post("../ajax/ajax_support.php", {
            step: 'remove_subs',
            tcid: tcid,
            ...
            if (!isNaN(stuff)) {
                ...
                alert("Opps, sorry! \n\nCouldn't not remove subscriber. \n\nPlease try again.");
            }
        });
    }
}

function delete_my_comment(cid){
    if (confirm("Are you sure?")) {
        $.post("../ajax/ajax_support.php", {
            step: 'delete_my_comment',
            cid: cid,
        }, function (stuff) {
            if (!isNaN(stuff)) {
                //load_my_tickets();
