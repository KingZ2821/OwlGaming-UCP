...
if (isset($_POST['type']) and isset($userID)) {
    if ($_POST['type'] == 2) { // unban
        if ($_POST['type2'] == "Forums") {
            ...
            if ($ban and is_numeric($ban['id'])) {
                ...
                if (is_null($ban['threadid']))
                    ...
                    $text .= "<li>Ban Thread: http://forums.owlgaming.net/index.php?showtopic=" . $ban['threadid'] . " </li>";
                $text .= "</ul></i>";
                echo $text;
                ?>
        ...
        if ($_POST['type2'] == "id") {
            ?>
            <form id="form_search_ticket" onsubmit="ticket_search_start('<?php echo htmlspecialchars($_POST['type2'], ENT_QUOTES, 'UTF-8'); ?>');
                                return false;">
                <table>
                    <tr>
